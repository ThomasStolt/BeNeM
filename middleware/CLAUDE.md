...
- `list_poll_seconds` (int, **default 30, min 15, max 900**; single home `config.LIST_POLL_DEFAULT`, mirrored in `benem-admin/servers.py`) — **C19, 2.21.0.** Interval between `getincidents` calls. ONE request per server per interval, whatever the incident count. Not portal-editable, but it round-trips a portal save — omitting it from `save_servers` would silently put every server back to 30 s.
- `cache_refresh_seconds` (int, default 120, min 60, max 900) — **the ENRICHMENT cadence.** Unchanged in meaning and deliberately NOT renamed in 2.21.0; the C6 rename to `incident_polling_seconds` is deferred to a migration that reads the old key, so a configured value cannot be lost
