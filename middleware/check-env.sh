...
# ── Credential reuse: no two secrets may share a value ───────────────────────
#
# Added 2026-09-15 after WEBHOOK_SECRET and PROXY_TOKEN were found set to the same
# 64-character value in a live deployment. Each is generated by its own
# `openssl rand -hex 32` in INSTALL.md §4, so identical values are always a
# copy-paste, never a coincidence — and the consequence is that the blast radius of
# whichever one leaks becomes the union of both. This checks the class, not that
# one pair.

echo "── Checking for reused credential values ──"

SECRET_KEYS="BENEM_SECRET_KEY SESSION_SECRET PROXY_TOKEN WEBHOOK_SECRET TOTP_SECRET \
VAPID_PRIVATE_KEY APNS_PRIVATE_KEY_B64 BASIC_AUTH_HASH"

reuse_found=0
for a in $SECRET_KEYS; do
  va="$(val "$a")"
  # Short or empty values are not credentials worth comparing; several are
  # legitimately absent and an empty-equals-empty match would be pure noise.
  [[ ${#va} -ge 16 ]] || continue
  for b in $SECRET_KEYS; do
    ...
    [[ ${#vb} -ge 16 ]] || continue
    if [[ "$va" == "$vb" ]]; then
      err "$a and $b have the SAME value — generate each separately (openssl rand -hex 32). Whichever leaks exposes both."
      reuse_found=1
    fi
  done
done
if [[ $reuse_found -eq 0 ]]; then ok "no credential value is reused"; fi
echo ""
