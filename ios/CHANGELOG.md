...
- **The connection test probes `incident_api.php`, the endpoint the app actually uses.** It previously probed `ha_status_api.php`, which on a TLS-terminated BHNM answers `200` with `a:1:{s:5:"Error";s:29:"API require HTTPS connection.";}` — PHP-serialized, not JSON — so **no connection could be saved at all**. The new probe sends a deliberately unsupported method: BHNM checks the credential *before* the method, so the reply is a constant **51 bytes** whatever the size of the estate, and it verifies reachability, proxy auth, key-target binding and the BHNM credential in one call. `getincidents` would answer the same question in ~209 bytes per incident, about **204 KB at n=1000**, and neither `limit` nor `count` bounds it. BHNM bug reported separately in `docs/evidence/2026-09-18-bhnm-ha-status-https-bug.md`.

- **A successful save now says what it verified.** It previously just dismissed, which is indistinguishable from nothing having happened. The confirmation names the two things the probe actually proves — **"BHNM is reachable through the middleware and accepted the credentials."** and returns to Settings on OK. "Credentials" rather than "API key" because the PIN is sent too whenever one is entered, so on a SaaS server it is verified as well.
- **An authentication failure quotes BHNM instead of guessing which credential is wrong.** BHNM never says which one it rejected, so the app no longer implies it: it shows BHNM's own words, then points at the right field using what the form already knows — **"Check the API key and the PIN."** when a PIN was entered, **"Check the API key. SaaS servers also require a PIN."** when none was.
- **The connection verdict has three outcomes, not two.** *Verified*, *authentication failed*, and **inconclusive** — a reply the app does not recognise is no longer treated as a pass. The previous rule hung the whole verdict on BHNM's exact wording of `Password failed.`, so a future rewording would have made a **wrong key read as a good connection**. Matching is now on structure first (`result: "completed"`) and the narrowest possible words second, credential words checked before anything else. An inconclusive result does not save.
