...
- **A successful save now says what it verified.** It previously just dismissed, which is indistinguishable from nothing having happened. The confirmation names the two things the probe actually proves — **"BHNM is reachable through the middleware and accepted the API key."** — and returns to Settings on OK.
